...
#region = "gene"; name = "NC_068449.1_esrb_minInd0.3_minDepthHalf"
region = "expand"; name = "NC_068449.1_s25414060_e25501622_esrb_minInd0.3_minDepthHalf"
...
pca1_2 <- ggplot(data = pca.vectors, aes(x=V1, y=V2, fill = Runtime, color = Runtime)) + 
  ...
  ggtitle(paste0(prefix," Chr 29 - esrb gene")) +
  labs(x = paste0("PC1 (",round(varPC1, digits = 2),"%)"), 
       y= paste0("PC2 (",round(varPC2, digits = 2),"%)")) 
pca1_2

if(region == "gene"){
ggsave(filename = paste0("./figures/pca/",prefix,"_esrb_pca1_2.jpeg"), plot = pca1_2,
       width = 8, height = 6)
}else{
ggsave(filename = paste0("./figures/pca/",prefix,"_expanded esrb_pca1_2.jpeg"), plot = pca1_2,
       width = 7, height = 6)
}
...
pca_cutoff <- ggplot(data = pca.vectors, aes(x=V1, y=V2, fill = Runtime, color = Runtime)) + 
  ...
  ggtitle("Coho aligned to chum: PCA of esrb gene") + 
  labs(x = paste0("PC1 (",round(varPC1, digits = 2),"%)"), 
       y= paste0("PC2 (",round(varPC2, digits = 2),"%)")) +
  theme(legend.position = "bottom")
pca_cutoff

ggsave(filename = paste0("./figures/pca/",prefix,"_",name,"_pca_allele_cutoff.jpeg"), plot = pca_cutoff,
       width = 5, height = 5)
...
hom_indivs <- allele_groups %>%
  filter(Genotype != "EL") %>% 
  mutate(name = sampleID) %>%
  subset(select = c(name, Genotype))

# write out to text file to be transferred and used as input
write.table(hom_indivs , file = paste0("./sedna_files/inputs/",prefix,"_esrb_allele_groups.txt"), 
...
coho_barplot <- ggplot(allele_props, aes(x = Runtime, y = n, fill = Genotype)) +
...
jpeg(file = paste0("./figures/pca/",prefix,"_esrb_barplot.jpg"), 
...
nrow(pca.vectors %>% filter(V1 < 0.1, V1 > -0.02))

allele_groups <- pca.vectors[,1:10] %>% 
  mutate(group = case_when(V1 > 0.1 ~ "early_al",
                             V1 < -0.02 ~ "late_al",
                             TRUE ~ "Het")) %>%
  filter(group != "Het") %>% 
  mutate(name = str_c("ABLG",ABLG)) %>%
  subset(select = c(name, group))

# write out to text file to be transferred and used as input
write.table(allele_groups , file = "./sedna_files/chumrun_esrb_allele_groups.txt", 
            col.names = F, row.names = F, sep = "\t", quote = F)
...
# write out to text file to be transferred and used as input
write.table(allele_groups , file = "./data/R/chumrun_esrb_allele_groups.txt", 
            col.names = T, row.names = F, sep = "\t", quote = F)
