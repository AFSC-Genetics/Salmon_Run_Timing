...
METADATAFILE <- "./data/raw/wood_runtiming_metadata_anvil_added.csv"
...
# call in bams
bam_df <- read.table(paste0("./sedna_files/bams/all_sockeye_bamslist.txt"), header = F)
...
Runtime <- c("Early Stream", "Late Stream", "Late Beach")
color <- c("goldenrod2","aquamarine4", "mediumpurple3")
...
ggsave(filename = paste0("./figures/pca/",prefix,"_minInd0.3_lrrc9_pca1_2.jpeg"), plot = pca1_2,
       width = 6, height = 6)
...
ggsave(filename = paste0("./figures/pca/lrrc9/",prefix,"_lrrc9_pca_allele_cutoff.jpeg"), plot = pca_cutoff,
       width = 5, height = 5)
...
write.table(allele_groups, file = paste0("./data/R/",prefix,"_lrrc9_alleles_all.txt"), 
            col.names = F, row.names = F, sep = "\t", quote = F)
# also add to fourspecies for barplots
write.table(allele_groups, 
            file = paste0("../2024_fourspecies/data/R/threespp_lrrc9_alleles/",prefix,"_lrrc9_alleles_all.txt"), 
            col.names = F, row.names = F, sep = "\t", quote = F)
...
# write out to text file to be transferred and used as input
write.table(hom_indivs, file = paste0("./sedna_files/inputs/",prefix,"_lrrc9_allele_groups.txt"), 
            col.names = F, row.names = F, sep = "\t", quote = F)

# only euclide data 
allele_groups2 <- pca.vectors %>%
  filter(Runtime != "Late Stream") %>%
  mutate(Genotype = case_when(V1 > EE_cutoff ~ "EE",  
                              V1 < LL_cutoff ~ "LL",
                              TRUE ~ "EL")) %>%
  filter(Genotype != "EL") %>%
...
write.table(allele_groups2, file = paste0("./sedna_files/inputs/euclideOnly_lrrc9_allele_groups.txt"), 
            col.names = F, row.names = F, sep = "\t", quote = F)
...
sock_barplot <- ggplot(allele_props, aes(x = Runtime, y = n, fill = Genotype)) +
  geom_bar(position = "fill", stat = "identity") +
  scale_fill_manual(values = c("khaki2", "lightpink", "brown4")) +
  theme_bw() +
  xlab("Sockeye") +
  theme(legend.position = "right",
...
jpeg(file = paste0("./figures/pca/",prefix,"_barplot.jpg"), 
