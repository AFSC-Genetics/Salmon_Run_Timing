...
# WHITEFISH Ck males (LATE) compared to TEAL Ck (EARLY, from Euclide et al., 2024)
...
METADATAFILE <- "./data/raw/wood_runtiming_metadata.csv"
MATURDATAFILE <- "./data/R/ABLG_pop_maturity.txt"
...
# prepare data
mat_df <- read.delim2(MATURDATAFILE, sep = "\t", header = TRUE, row.names = NULL) %>%
  mutate(sampleID = paste0("ABLG",ABLG))
head(mat_df)

run_df <- read.csv(METADATAFILE)

## ADD THE MATURITY DATA TO FILTER OUT JACKS
full_metadata <- left_join(run_df, mat_df, by = "sampleID") %>%
  filter(maturity == "normal" | is.na(maturity))
head(full_metadata)
...
keeplist_df1 <- depths_df[depths_df$mean_depth >= 1,]
blocklist_df1 <- depths_df[depths_df$mean_depth < 1,]
...
factors_list <- unique(keeplist_df1[["feature"]])
