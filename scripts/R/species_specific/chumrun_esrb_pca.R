...
pca1_2 <- ggplot(data = pca.vectors, aes(x=V1, y=V2, fill = Runtime, color = Runtime)) + 
  ...
  ggtitle("Chum: Chr 29 - esrb gene region") + 
  labs(x = paste0("PC1 (",round(varPC1, digits = 2),"%)"), 
       y= paste0("PC2 (",round(varPC2, digits = 2),"%)")) 
pca1_2

ggsave(filename = paste0("./figures/pca/esrb/",prefix,"_",name,"_pca1_2.jpeg"), plot = pca1_2,
       width = 7, height = 6)
...
pca_cutoff <- ggplot(data = pca.vectors, aes(x=V1, y=V2, fill = Runtime, color = Runtime)) + 
  ...
  ggtitle("Chum: PCA of esrb gene") + 
  labs(x = paste0("PC1 (",round(varPC1, digits = 2),"%)"), 
       y= paste0("PC2 (",round(varPC2, digits = 2),"%)")) +
  theme(legend.position = "bottom")
pca_cutoff

ggsave(filename = paste0("./figures/pca/esrb/",prefix,"_",name,"_pca_allele_cutoff.jpeg"), plot = pca_cutoff,
       width = 5, height = 5)
...
write.table(hom_indivs , file = paste0("./sedna_files/",prefix,"_esrb_allele_groups.txt"), 
...
# write out to text file to be transferred and used as input
write.table(allele_groups, file = paste0("../2024_fourspecies/data/R/twospp_esrb_alleles/chumrun_esrb_alleles.txt"), 
            col.names = F, row.names = F, sep = "\t", quote = F)

allele_props <- pca.vectors[,1:10] %>% 
  mutate(Genotype = case_when(V1 < EE_cutoff ~ "EE",  
                              V1 > LL_cutoff ~ "LL",
                              TRUE ~ "EL")) %>% 
  mutate(name = str_c("ABLG",ABLG)) %>%
...
write.table(allele_props , 
            file = paste0("./data/R/",prefix,"_allele_proportions.txt"), 
            col.names = T, row.names = F, sep = "\t", quote = F)

# PLOT
chum_barplot <- ggplot(allele_props, aes(x = Runtime, y = n, fill = Genotype)) +
  geom_bar(position = "fill", stat = "identity") +
  scale_fill_manual(values = c("goldenrod2", "green3", "dodgerblue4")) +
  theme_bw() +
  xlab("Chum") +
  theme(legend.position = "right",
...
jpeg(file = paste0("./figures/pca/",prefix,"_barplot",name,".jpg"), 
