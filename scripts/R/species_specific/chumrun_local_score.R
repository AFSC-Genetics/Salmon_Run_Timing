...
filename="chumrun_minInd0.3_minDepthHalf_MM3"
...
chromfile <- read_tsv(here("data/R/chrom_meta.txt"), show_col_types = F)
...
chum_list <- as.list(Sys.glob("./results/fst/*chumrun*fall-summer_minInd0.3_minDepthHalf*fst.txt"))
...
fst_localpeaks <- ggplot() +
  geom_point(data = chum_df, aes(x=midPos, y=Fst, color = as_factor(chr))) +
  ...
  scale_color_manual(values = rep(c("gray50","black"),length(unique(chum_df$chr))))+
...
ggsave(paste0("./figures/localscore/chumrun_fst_localscore_xi",xi,"_sig",alpha,".png"),
