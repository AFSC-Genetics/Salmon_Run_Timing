...
wgd_genes <- read.csv("./data/R/wgd_geneviewer_data_er1.csv", row.names = NULL) %>%
  filter(duplication < 3)
...
GC_chart(wgd_genes, 
  ...
  GC_genes(marker_size = "large", marker = "box", markerHeight = 30,
           customColors = c("goldenrod","green")
  ) %>%
  ...
  GC_scaleBar(cluster = 1, x = 0, y = 168,
              ...
              title = "230kb", scaleBarUnit = 230000) %>%
  GC_scaleBar(cluster = 2, x = 0, y = 10,
              ...
              scaleBarLineStyle = list(stroke = "black", strokeWidth = 1.5),
              scaleBarTickStyle = list(stroke = "black", strokeWidth = 1.5),
              title = "174kb", scaleBarUnit = 174000)


# SECOND WITHOUT EXTRA STUFF
GC_chart(wgd_genes, 
         cluster = "chr",
         group = "duplication",
         height = 360) %>%
...
# for esr1
wgd_genes_small <-  read.csv("./data/R/wgd_geneviewer_data_er1.csv", row.names = NULL) %>%
  filter(duplication != 1)

GC_chart(wgd_genes_small, 
         cluster = "chr",
         group = "duplication",
         #style = list(border = "5px solid red"),
         height = 100) %>%
  GC_align(id_column = "Gene", id = "fake", align = "right"
  ) %>%
  GC_genes(show = T,
           marker_size = "small", marker = "box",
           customColors = c("green","white","white","white")
  ) %>%
  GC_links(group = "duplication", 
           value1 = c(2,3),
           value2 = c(2,3),
